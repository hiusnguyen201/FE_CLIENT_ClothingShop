...
const CartItems: React.FC<CartItemsProps> = ({ productsData }) => {
  const colorProduct = ["White", "Black", "Green"];
  const sizeProduct = ["S", "M", "L", "Xl", "2Xl"];

  //toggle select all item
  const [selectedItems, setSelectedItems] = useState<string[]>([]);
  const isAllSelected = selectedItems.length === productsData.length;

  const handleToggleAll = () => {
    if (isAllSelected) {
      setSelectedItems([]);
    } else {
      setSelectedItems(productsData.map((item) => item._id));
    }
  };
    ...
    <div className="w-full max-w-screen-lg mx-auto px-4">
      <div className="flex justify-between items-center border-b border-gray-200 my-5">
        <div className="flex items-center space-x-2">
          ...
          <Button className="shadow-none text-sm md:text-base">All Product</Button>
          <span>|</span>
          <Button className="shadow-none text-sm md:text-base">Clear</Button>
        </div>
        ...
        >
          ...
          <div className="flex items-center gap-4 w-full lg:w-3/4">
            ...
            <div className="w-28 sm:w-32 md:w-36">
              <img src={item.image} alt="product" className="w-full aspect-[3/4] object-cover rounded-md" />
            </div>

            <div className="flex flex-col w-full gap-2">
              <h4 className="font-medium text-sm md:text-base lg:text-lg">{item.name}</h4>
              ...
              <div className="flex flex-wrap items-center gap-2 relative">
                ...
                >
                  ...
                  <i
                    className={`ri-arrow-down-s-line transition-transform duration-300 ${
                      openDropdownColorIndex === index ? "rotate-180" : ""
                    }`}
                  ></i>
                </div>
                      ...
                      {colorProduct.map((option, colorIdx) => (
                        <li
                          ...
                          className={`px-4 py-2 cursor-pointer hover:bg-gray-100 ${
                            selectedColor[index] === option ? "bg-gray-100" : ""
                          }`}
                        >
                          {option}
                        </li>
                ...
                >
                  ...
                  <i
                    className={`ri-arrow-down-s-line transition-transform duration-300 ${
                      openDropdownSizeIndex === index ? "rotate-180" : ""
                    }`}
                  ></i>
                </div>
                    ...
                    <ul className="py-2 text-sm">
                      {sizeProduct.map((option, sizeIdx) => (
                        <li
                          ...
                          className={`px-4 py-2 cursor-pointer hover:bg-gray-100 ${
                            selectedSize[index] === option ? "bg-gray-100" : ""
                          }`}
                        >
                          {option}
                        </li>
              ...
              <button className="flex items-center text-red-500 text-sm hover:underline mt-2">
