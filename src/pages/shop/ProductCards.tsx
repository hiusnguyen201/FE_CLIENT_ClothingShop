...
import { SizeBadge } from "@/components/productSlice/SizeBadge";
...
const ProductCards: React.FC<ProductCardProps> = ({ productsData }) => {
  const controlsArray = productsData.map(() => useAnimation());
            ...
            <div key={index} className="product__card">
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(product.sizes || ["S", "M", "L", "XL", "2XL"]).map((size, i) => (
                      <SizeBadge key={i} title={size} className="w-[40px] h-[38px] bg-white hover:bg-gray-200" />
                    ))}
                  </div>
                </motion.div>
              </motion.div>
              ...
              <div className="flex flex-col min-h-[94px]">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {product.discount && (
                    <p className="text-white p-1 bg-red-500 md:flex hidden rounded-xl font-bold text-sm">
                      -{product.discount}
                    </p>
                  )}
                </div>
