...
const NavBar: React.FC = () => {
  ...
  const { user, loading: userLoading } = useAppSelector((state) => state.account);
            ...
            >
              ...
              <div className="flex flex-col w-2/3 border-r">
                <div className="p-6 flex gap-10">
                  {categoryList.map((category) => (
                    <ul key={category.name}>
                      <Link to={`${category.path}`} className=" hover:text-red-500 text-xl flex items-center gap-2">
                        {category.name}
                        <ArrowRightIcon />
                      </Link>
                      {category.subCategories?.map((subCategory) => (
                        <li key={subCategory.name} className="py-3 text-stone-600 hover:text-red-500 text-sm">
                ...
                <div className="mt-auto bg-slate-100 px-6 py-4 text-xl flex flex-row gap-4">
                  <div className="text-stone-600 border-r border-gray-200">According To Need</div>

                  {categoryAccordingToNeed.map((c, index) => (
                    <Link key={index} to={`/${c.path}`} className=" hover:text-red-500">
                      {c.name}
                    </Link>
                  ))}
                </div>
              ...
              <div className="flex flex-col w-1/3 p-8 gap-2">
                <div className="relative">
                  <img
                    ...
                    alt=""
                  />
                  <span className="absolute bottom-0 m-2 font-bold text-black text-sm">Men's Ultra Light Jeans</span>
                </div>
        ...
        <div className="hidden md:flex flex-row items-center gap-6">
          ...
          <Link to={"/cart"} className="relative">
            ...
            <span className="text-xs absolute -right-2 top-3 px-1 text-white rounded-full bg-red-500">
              {!userLoading.getProfile && !cartLoading.getCart && cart.length}
            </span>
          </Link>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-slate-200">
                  <div className="bg-white border rounded-lg m-4 px-2 ">
                    <div className="text-lg py-2 uppercase">Men</div>
                    <Accordion type="single" collapsible className="w-full">
                      {categoryList.map((category) => (
                        <AccordionItem key={category.name} value={category.name} className="border-none">
                          <AccordionTrigger className="hover:no-underline py-2">{category.name}</AccordionTrigger>
                          <AccordionContent className="px-3 flex flex-col gap-4">
                            <Link to={category.path}>{category.name} list</Link>
                            {category.subCategories.map((subCategory) => (
                              <Link to={subCategory.path} key={subCategory.name}>
                                {subCategory.name}
                              </Link>
                            ))}
                          </AccordionContent>
